...
parser <- add_option(parser, c("-n", "--normalization"), default = "funnorm", help = "normalization method")
...
performPreprocessing <- function(rg.set, preprocess.method = "") {
  if (preprocess.method == "funnorm") {
    write("Perform background subtraction with dye-bias normalization and infer between-array technical variation.", stdout())
    m.set <- preprocessFunnorm(rg.set)
  } else if (preprocess.method == "quantile") {
    write("Perform stratified quantile normalization.", stdout())
    m.set <- preprocessQuantile(rg.set)
  } else if (preprocess.method == "raw") {
    write("Preprocessing without normalization.", stdout())
    m.set <- preprocessRaw(rg.set)
  } else {
    write("Preprocessing method not recognized. Available methods: noob, raw.", stdout())
    return()
  }
  ...
  if (class(m.set) == "MethylSet") {
    ...
    # Generate QC dropLociWithSnps
    qc <- getQC(m.set)
    plotQC(qc)

    phenoData <- pData(rg.set)
    if (phenoData$Sample_Group) {
...
if (args$outputtype == "beta") {
  ...
  write("Output file format not recognized. Available formats: 'beta','m-values'", stdout())
  return()
}
